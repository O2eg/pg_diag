...
select
    ...
    case
        when coverage.candidate_sample_truncated or coverage.acl_expansion_truncated or coverage.result_truncated
            then 'Direct grant is part of a truncated bounded sample; review coverage flags before treating the inventory as complete'
        else 'Privilege is granted directly to a login role in the bounded object sample; whether group roles are required depends on the access-control baseline'
    end as risk_reason
from findings
cross join coverage
union all
select
    ...
    coverage.result_truncated,
    'unknown'::text,
    'The bounded object candidate, ACL expansion, or result sample was truncated; absence of direct-grant findings is not a clean result'::text
from coverage
