select
  ...
  case when wal_status = 'lost' then 'high' else 'ok' end as pg_diag_internal_severity,
  case when wal_status = 'lost' then 'replication slot has lost required WAL' else '' end
