with recovery as (
  select
    ...
    case when not pg_catalog.pg_is_in_recovery() then 'not in recovery' when pg_catalog.pg_is_wal_replay_paused() then 'paused' else 'not paused' end as replay_pause_state
),
receiver as (
  select
...
settings as (
  select
    null::text as primary_conninfo_setting,
    null::text as primary_slot_name,
    null::text as recovery_min_apply_delay,
    null::text as recovery_target_timeline,
    null::boolean as restore_command_configured,
