with sessions as (
  select
    ...
    s.bits as tls_bits,
    g.encrypted as gss_encrypted
  from pg_catalog.pg_stat_activity a
  left join pg_catalog.pg_stat_ssl s on s.pid = a.pid
  left join pg_catalog.pg_stat_gssapi g on g.pid = a.pid
...
per_role_bounded as (
  select
    ...
    count(*) filter (where c.gss_encrypted)::int8 as gss_encrypted_session_count,
    ...
    count(*) filter (
      where c.client_addr is not null and not c.is_loopback
        and c.ssl = false and coalesce(c.gss_encrypted, false) = false
