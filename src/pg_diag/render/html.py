...
def render_html(artifact: dict[str, Any], *, validate: bool = True) -> str:
    ...
    replacements = {
        ...
        "__PG_DIAG_GRAPH_JS__": _inline_script("\n".join(
            ...
            for name in ("pg-diag-graph-data.js", "pg-diag-graph-rules.js", "pg-diag-graph-groups.js", "pg-diag-graph.js")
        )),
...
def _publicize_artifact_for_render(artifact: dict[str, Any]) -> dict[str, Any]:
    ...
    visible_item_ids = {
        item_id for section in public_sections for item_id in section.get("items") or []
    }
    public_artifact["items"] = deepcopy(
        {item_id: item for item_id, item in all_items.items() if item_id in visible_item_ids}
    )
