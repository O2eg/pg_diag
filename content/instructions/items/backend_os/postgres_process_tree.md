...
- A local `ps fax` forest snapshot filtered to PostgreSQL server processes.
- PID, PPID, user, process state, elapsed time, CPU percent, memory percent, and command text.
- Parent-child relationships between the postmaster process, client backends, background workers, WAL processes, autovacuum workers, and parallel workers when they are visible in the OS process tree.
...
- Re-run a fresh local snapshot before terminating processes, because `ps fax` is a point-in-time view.
