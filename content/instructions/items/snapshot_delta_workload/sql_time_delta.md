...
## Interval coverage
- Global `pg_stat_statements_info.stats_reset` is checked on PostgreSQL 14-18; PostgreSQL 17+ also checks per-entry `stats_since`.
- `epoch_changed` or a counter decrease is invalid coverage and is omitted.
- `missing_start`/`missing_end` is expected bounded-selection churn, not zero activity. On PostgreSQL 14-16 entry eviction has no per-entry epoch and may only appear as churn or a decrease.
