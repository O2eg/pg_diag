with workers as (
  select
    ...
    s.subname,
    s.subenabled,
    case when w.pid is null then 'not running' else w.worker_type end as worker_type,
    w.pid,
    w.leader_pid,
    w.relid,
    ...
    ss.sync_error_count,
    case
      when ss.subid is null then null
      else (
...
select
  ...
  case
    ...
    when apply_error_count > 0 or sync_error_count > 0 or conflict_count > 0 then 'medium'
    else 'ok'
  end as pg_diag_internal_severity,
  case
    when subenabled and pid is null then 'enabled subscription apply worker is not running'
    when apply_error_count > 0 or sync_error_count > 0 or conflict_count > 0
