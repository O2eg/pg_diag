with reset_events as (
  ...
  union all
  select 'pg_stat_checkpointer', 'cluster', 'checkpointer', stats_reset
  from pg_stat_checkpointer
  union all
  select 'pg_stat_archiver', 'cluster', 'archiver', stats_reset
  from pg_stat_archiver
  union all
  select 'pg_stat_wal', 'cluster', 'wal', stats_reset
  from pg_stat_wal
  union all
  select 'pg_stat_slru', 'slru', name::text, stats_reset
  from pg_stat_slru
  union all
  select 'pg_stat_recovery_prefetch', 'recovery', 'prefetch', stats_reset
  from pg_stat_recovery_prefetch
  union all
  select 'pg_stat_replication_slots', 'replication_slot', slot_name::text, stats_reset
  from pg_stat_replication_slots
  union all
  select
    'pg_stat_io',
    'io',
    'all backend/object/context rows',
    min(stats_reset)
  from pg_stat_io
),
normalized as (
