...
async def collect(ctx: PythonSourceContext) -> PythonSourceResult:
    ...
    for path in _sudoers_files():
        ...
        for line_number, line in enumerate(text.splitlines(), start=1):
            ...
            if "postgres" not in lower:
                continue
            risky = "nopasswd" in lower or "all" in lower or re.search(r"(^|/)(su|sudo|bash|sh)\b", lower)
            if risky:
                rows.append(
                    {
                        "file_path": str(path),
    ...
    if not read_any and unreadable_paths:
        return _unavailable_result(
            "The collector cannot read sudoers files to verify postgres escalation rules",
