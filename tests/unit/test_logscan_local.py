...
def _request(tmp_path, files, window_from: datetime, window_to: datetime | None = None,
             **kwargs) -> ScanRequest:
...
def test_local_scan_filters_and_collapses_flood(tmp_path) -> None:
    ...
    result = asyncio.run(
        LocalLogSource(str(tmp_path)).scan(_request(tmp_path, [info], BASE))
    )
...
def test_local_scan_wire_budget_truncates(tmp_path) -> None:
    ...
    result = asyncio.run(
        LocalLogSource(str(tmp_path)).scan(
            _request(tmp_path, [info], BASE, wire_budget_bytes=4096)
        )
    )
...
def test_local_scan_wire_budget_is_hard(tmp_path) -> None:
    ...
    result = asyncio.run(
        LocalLogSource(str(tmp_path)).scan(
            _request(tmp_path, [info], BASE, wire_budget_bytes=1)
        )
    )
    assert result.series == []
    assert "return_limit_hit" in result.stats.truncation_reasons
...
def test_local_scan_refuses_symlink_escape(tmp_path) -> None:
    ...
    result = asyncio.run(
        LocalLogSource(str(logdir)).scan(_request(logdir, [info], BASE))
    )
    assert result.series == []
