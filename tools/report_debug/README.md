...
| `check_routes.cjs` | Пунктирные связи и связи родитель → потомок: полные, частичные и смешанные раскрытия, карточки разной высоты |
...
`navigation.py --icons` дополнительно сравнивает иконки типов в карточке и
заголовке айтема, проверяет расположение справа от текста и по центру строки.
...
Раздел `tree` проверяет сплошные связи на смешанных раскрытиях с карточкой
каждого видимого узла. Флаг `browser_audit.py --edges` дополнительно проверяет
реальные сплошные SVG-пути: скругления, порты, пересечения и видимость после
анимации. Без `--node` он перебирает все узлы графа; с `--node` можно повторить
конкретную ветку. `--edges` включает также проверку пунктирных связей.
`--arrow-zoom` измеряет фактически нарисованный наконечник и толщину причинной стрелки
при масштабах 0.25×, 0.5×, 1× и 2×. Используется SVG-маркер текущего отчёта
и стили линии; проверяется пропорциональное изменение размеров при зуме.
`--center-cards` проверяет центрирование открываемых карточек без изменения
масштаба; с `--node ID` можно выбрать конкретные узлы. Снимки `--screenshots`
в этом режиме сохраняют результат центрирования без дополнительного Fit.
`--node-links` проверяет обычными кликами переходы Related checks, Possible causes
и Possible effects в обоих направлениях. Проверяются раскрытие скрытого узла,
центрирование карточки без изменения масштаба и отсутствие скачка перед
анимацией: обе темы, обычное и сокращённое движение, масштабы 0.5× и 1.64×.
`--node ID` ограничивает начальные узлы переходов. Общий сценарий находится
в `browser_interactions.py` и используется также браузерными регрессионными тестами.
`--screenshots DIR` сохраняет снимки выбранных ветвей в тёмной теме после Fit,
чтобы сравнить расположение и вид соединений до и после изменения алгоритма.
`--facts` проверяет таблицы фактов во всех карточках в обеих темах: границы,
отступы, пропорции колонок, переполнение текста и обрезание карточки.
Также проверяются положение крестика в правом верхнем углу заголовка
и отсутствие его наложения на текст.
С `--node ID --screenshots DIR` также сохраняет отдельные снимки выбранных
карточек в масштабе 1:1 для проверки оформления.
`--explain-button` проверяет переход из кнопки Explain available в шапке:
обычный клик, раскрытие раздела и айтема, сброс скрывающего фильтра, прокрутку
и фокус в обеих темах. С `--screenshots DIR` сохраняет также снимки шапки.
`--all-details` проверяет также второй шаг Expand all: карточки всех узлов,
закрытие одной карточки и маршруты между карточками разной высоты. Используйте
`--node ID`, чтобы ограничить проверку выбранными узлами.
...
Для точечного переноса правок самого шаблона (например, кнопки в шапке)
добавьте `--template-before /path/to/template-before.html`: инструмент применит
только отличия от текущего `src/pg_diag/render/templates/report.html`.
Предыдущую версию шаблона сохраните до редактирования. Каждый фрагмент должен
иметь единственное совпадение в отчёте; изменения, затрагивающие подстановки
данных или библиотек, отклоняются. Резервная копия по-прежнему обязательна.
